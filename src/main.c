...
{
    GLFWwindow *window;
    if (!(window = seel_create_window(WINDOW_WIDTH, WINDOW_HEIGHT, "Seaeel Engine v0.1")))
    {
        fprintf(stderr, "Failed to creating engine window.\n");
        return -1;
    };
    ...
    struct AssetManager manager = seel_asset_manager_create();

    if (!SEEL_ASSET_MANAGER_LOAD(&manager, ASSET_SHADER, "default", "../shaders/default.vert", "../shaders/default.frag"))
        return -1;
    if (!SEEL_ASSET_MANAGER_LOAD(&manager, ASSET_SHADER, "lights", "../shaders/lights.vert", "../shaders/lights.frag"))
        return -1;
    if (!SEEL_ASSET_MANAGER_LOAD(&manager, ASSET_SHADER, "text", "../shaders/text.vert", "../shaders/text.frag"))
        return -1;

    if (!SEEL_ASSET_MANAGER_LOAD(&manager, ASSET_MODEL, "vampire", "../assets/models/vampire/dancing_vampire.dae"))
        return -1;

    struct Shader *dft = SEEL_ASSET_MANAGER_GET(&manager, SHADER, "default");
    struct Shader *lights = SEEL_ASSET_MANAGER_GET(&manager, SHADER, "lights");
    struct Shader *text = SEEL_ASSET_MANAGER_GET(&manager, SHADER, "text");

    struct Model *vampire = SEEL_ASSET_MANAGER_GET(&manager, MODEL, "vampire");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        seel_render_text(*text,
                         "Seaeel Engine 0.1v", 10.0f, tex_start_y - 25.0f, 0.5f, (vec3){1.0f, 1.0f, 1.0f}, width, height);
        char fps_text[32];
    ...
    seel_asset_manager_cleanup(&manager);

    // nk_glfw3_shutdown();
    seel_destroy_window(window);
    return 0;
}
...
void seel_process_input(GLFWwindow *window, float delta_time)
{
    ...
    if (keys[GLFW_KEY_1])
    {
        enable_directional_light = !enable_directional_light;
    }

    if (keys[GLFW_KEY_2])
    {
        enable_point_lights = !enable_point_lights;
    }

    if (keys[GLFW_KEY_3])
    {
        enable_spot_light = !enable_spot_light;
    }
